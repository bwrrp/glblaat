...
GLFramebuffer::~GLFramebuffer() {
	...
	// TODO: add reference counting to prevent deleting twice!
	// Packed depth/stencil buffers are attached twice
	if (attachments[GL_DEPTH_ATTACHMENT_EXT] == attachments[GL_STENCIL_ATTACHMENT_EXT]) {
		DetachRendertarget(GL_STENCIL_ATTACHMENT_EXT);
	}
...
void GLFramebuffer::AttachRendertarget(int attachment, GLRendertarget &rt) {
	if (!bound) Bind();
...
GLRendertarget* GLFramebuffer::DetachRendertarget(int attachment) {
	if (!bound) Bind();
	// It doesn't matter which function we use to detach this
	glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, 
		attachment, GL_RENDERBUFFER_EXT, 0);
	GLUtility::CheckOpenGLError("GLFramebuffer: DetachRendertarget()");
